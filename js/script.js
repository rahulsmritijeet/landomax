...
function showNotification(message, type = 'success') {
    ...
    notif.innerHTML = '<span>' + icon + ' ' + message + '</span>' +
...
function getTypeIcon(type) {
    const icons = {
        'microcontroller':'🎛️','sensor':'📡','motor':'⚙️','led':'💡','resistor':'🔧','capacitor':'🔋',
        'wire':'🔌','display':'📺','module':'📦','board':'🎚️','battery':'🔋','switch':'🔘'
    };
...
function updateComponentStats() {
    ...
    const lowStock = allComponents.filter(c => {
        const q = parseInt(c.Quantity)||0; return q > 0 && q <= 5;
    }).length;
...
async function confirmImport() {
    ...
    showLoading();
    try {
        // Use smart import (merges by name)
        const result = await apiCall('importComponentsFromExcel', { data: importData });
        showNotification('Imported. Added: ' + (result.added||0) + ', Merged: ' + (result.merged||0));
        closeImportModal();
        loadComponents();
    } catch(e){ console.error(e); }
    hideLoading();
...
async function loadProjects() {
    ...
    try {
        ...
        if (container) container.innerHTML = '<div class="empty-state"><h3>Failed to load</h3><button onclick="loadProjects()" class="btn btn-primary">Retry</button></div>';
    }
    hideLoading();
}
...
function viewProject(id) {
    ...
    html += '<div class="detail-section"><h4>👥 Team</h4><p>'+ (escapeHtml(p.TeamMembers)||'No team') +'</p></div>';
...
async function deleteProject(id){
    ...
    showLoading();
    try{ await apiCall('deleteProject',{id}); showNotification('Project deleted'); loadProjects(); }
    catch(e){ console.error(e); }
    hideLoading();
}
...
function toggleComponent(id) {
    ...
    if (!comp || (parseInt(comp.Quantity)||0)===0) { showNotification('Cannot add - out of stock!','error'); return; }
...
function renderSelectedComponents() {
    ...
    keys.forEach(id => {
        ...
        html += '<div class="selected-component-item '+(over?'over-stock-warning':'')+'">'+
            ...
            '<button type="button" class="qty-btn" onclick="changeComponentQty(\''+id+'\',1)">+</button></div>'+
            '<button type="button" class="remove-btn" onclick="removeComponent(\''+id+'\')">✕</button></div>';
    });
    c.innerHTML = html;
...
function searchComponentsInForm(query){
    ...
    document.querySelectorAll('#availableComponentsList .component-item').forEach(item=>{
        ...
        const matchType = currentTypeFilter==='all' || (item.dataset.type||'')===currentTypeFilter.toLowerCase();
        item.style.display = (matchSearch && matchType)?'flex':'none';
    });
}
function clearComponentSearch(){
    const i = document.getElementById('componentSearch');
    if (i){ i.value=''; searchComponentsInForm(''); }
}
function filterByType(type){
    currentTypeFilter = type;
    document.querySelectorAll('.quick-filters .filter-chip').forEach(ch=>ch.classList.remove('active'));
...
async function saveProject(event){
    ...
    if (error) return;
    const compsStr = Object.entries(selectedProjectComponents).map(([id,comp])=> id+':'+(comp.selectedQuantity||1)).join(', ');
    const data = {
...
async function loadCompetitions(){
    ...
    try {
        ...
        if (tbody) tbody.innerHTML='<tr><td colspan="7" class="no-data">Failed to load</td></tr>';
    }
    hideLoading();
}
...
function renderCompetitionsTable(list){
    const tbody=document.getElementById('competitionsTableBody');
    if (!tbody) return;
    if (!list.length){ tbody.innerHTML='<tr><td colspan="7" class="no-data">No competitions found</td></tr>'; return;}
    let html='';
    list.forEach(c=>{
        const status=(c.Status||'upcoming').toLowerCase();
        html+='<tr><td><code>'+c.EventID+'</code></td>'+
...
function toggleView(view,btn){
    const tv=document.getElementById('tableView'); const cv=document.getElementById('calendarView');
...
function closeCompetitionModal(){ document.getElementById('competitionModal').classList.remove('show'); }
function editCompetition(id){ window.location.href='competition_form.html?id='+id; }
async function deleteCompetition(id){
    if (!confirm('Delete this competition?')) return;
    showLoading();
    try{ await apiCall('deleteCompetition',{id}); showNotification('Competition deleted'); loadCompetitions(); }
    catch(e){ console.error(e); }
    hideLoading();
}
...
function createOrderCard(o){
    ...
    if (!totalQty) items.forEach(it=> totalQty+=parseInt(it.Quantity)||0);
    ...
    if (!isCompleted) html+='<button class="btn btn-success" onclick="openCompleteOrderModal(\''+o.OrderID+'\')">✅ Complete & Add</button>';
    else html+='<span class="completed-badge">✅ Added to Inventory</span>';
...
function closeViewOrderModal(){ document.getElementById('viewOrderModal').classList.remove('show'); }
function editOrder(id){ window.location.href='order_form.html?id='+id; }
async function deleteOrder(id){
    if (!confirm('Delete this order?')) return;
    showLoading();
    try{ await apiCall('deleteOrder',{id}); showNotification('Order deleted'); loadOrders(); }
    catch(e){ console.error(e); }
    hideLoading();
}
...
function closeCompleteOrderModal(){ document.getElementById('completeOrderModal').classList.remove('show'); currentCompleteOrderId=null; }
async function confirmCompleteOrder(){
    ...
    try{
        ...
        if (r.success){ showMergeResults(r); loadOrders(); }
...
function closeMergeResultsModal(){ document.getElementById('mergeResultsModal').classList.remove('show'); }
...
async function confirmOrderImport(){
    if (!orderImportData || !orderImportData.length){ showNotification('No items','warning'); return;}
    ...
    if (id){
        ...
        try{
            ...
            if (r.success && r.data){
                ...
                orderItems=(o.Items||[]).map(it=>({ComponentName:it.ComponentName,Type:it.Type||'',Quantity:parseInt(it.Quantity)||1}));
            }
        } catch(e){ console.error(e); }
        hideLoading();
    }
...
function updateOrderTotals(){
    ...
    let totalQty=0; orderItems.forEach(i=> totalQty+=parseInt(i.Quantity)||0);
